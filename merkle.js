...
<>
<Head>
<title>  Merkle Root and Proof Generator Tool </title>
</Head>
<div className="flex flex-col items-center">
  
<div className="h-12 bg-black-800 w-full flex flex-col items-center justify-center text-black font-bold text-md md:text-2xl">
<Link href="/"><a>🌱🌱🌱 Merkle Root and Proof Generator Tool 🔎🧾🔎</a></Link>
</div>
<div className="md:w-1/2">
<div className="text-sky-700 text-center p-2">
    Generate a Merkle Root Hash for NFT smart contracts and other use cases like airdropping users etc.Have fun 😜😜 
...
{theRootHash!=null && <p onClick={() => {navigator.clipboard.writeText(theRootHash);toast.success(`Copied!`)}} className="text-sky-700 border border-1 border-sky-700 cursor-pointer rounded-lg p-2">Copy Hash</p>}
    ...
    <form className="flex flex-col items-center justify-center m-2" onSubmit={handleSubmit(generateMerkleTree)}>
      ...
      <textarea className="w-full bg-black-800 m-2 border-2 p-2 text-sky-600 border-sky-700 text-lg rounded-lg" rows={10}
...
Found a bug? <br/> Let me know via telegram: <Link  href="https://t.me/abhishek_thaku_r"><a className="font-bold">Here 💖</a></Link>

</div>


<div className="text-center text-sky-700">
  Building NFT smart contracts? Feel free to contact me: 

  <div className="text-center text-sky-700 font-semibold underline">
<Link href='https://t.me/abhishek_thaku_r'>Telegram</Link>
</div>
<div>
  <Link href='Abhishek_thaku_r#5858'><a className="underline text-center font-semibold text-sky-700 cursor-pointer">Discord @Abhishek_thaku_r#5858 </a></Link></div>
  </div>
 </div>
</div>
</>
